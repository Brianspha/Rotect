...
contract RotectDAO is Ownable {
    ...
    function vote(
        ...
        pvote.inSupport = vote;
        pvote.justification = reason;
        pvote.voter = msg.sender;
        pvote.power = votePower;
        p.voters[msg.sender] = true;
        p.currentResult = (vote)
            ? p.currentResult.add(int256(votePower))
            : p.currentResult.sub(int256(votePower));
        token.increaseLockedAmount(daoOwner, votePower);
        emit Voted(msg.sender, vote, votePower, reason);
        ...
        )
    {
        return (
            ...
            proposals[projectSubmissions[id].index].votes
        );
    }
