...
contract RotectDAO is Ownable {
    ...
    event ProjectSubmissionCreated(uint256 index, bytes id, uint256 minFunds, uint256 maxFunds, address submitter, State state);
    event ProjectSubmissionDeleted(uint256 index, bytes id, address submitter, State state);
    ...
    event ProposalAdded(uint256 id, uint8 proposalType, bytes hash, string description, address submitter);
    ...
    function addProjectSubmission(bytes memory id, uint256 minFunds, uint256 maxFunds) public payable {
        require(token.balanceOf(msg.sender) >= 10 ** token.decimals(), "insufficient token balance");
        require(minFunds > 0, "Invalid min project funds");
        require(maxFunds > 0 && minFunds < maxFunds && minFunds != maxFunds, "Invalid max project funds");
    ...
    function propose(bytes memory id, string memory reason, uint8 proposalType) daoActive memberOnly public {
        require(submissionExists(id), "Submission not found");
        require(proposalType != 1 || proposalType != 2, "Invalid proposal type");
    ...
    function executeProposal(uint256 _id) daoActive public {
        Proposal storage proposal = proposals[_id];
        require(now >= proposal.deadline && !proposal.executed, "proposal hasnt expired");
    ...
    function vote(uint256 _proposalId, bool _vote, string memory reason, uint256 _votePower) daoActive tokenHolderOnly public returns(int256) {
        require(_votePower > 0, "cant vote");
        require(uint256(_votePower) <= token.balanceOf(msg.sender), "insufficient token balance");
    ...
    function deleteProjecSubmission(bytes memory id) daoActive internal returns(bool) {
        require(submissionExists(id), "Submission not found");
    ...
    function calculateProjectSubmissionFee() view internal returns(uint256) {
        return submissionZeroFee.mul(nonDeletedSubmissions);
    }

    function blacklistAddress(address _offender) daoActive internal {
    ...
    function removeFromBlackListMsgSender() payable public {
        removeFromBlackList(msg.sender);
    }

    function removeFromBlackList(address _offender) daoActive payable public {
    ...
    function whitelistAddress(address user) daoActive public payable {
    ...
    function updateTokenAddress(address _token) daoActive onlyOwner public {
        require(_token != address(0), "invalid address");
        token = RotectToken(_token);
        emit TokenAddressChange(_token);
    }

    function notVoting(address _voter) daoActive internal view returns(bool) {
    ...
    function getProjectSubmission(bytes memory id) public view returns(address, State, uint, uint256, uint256,uint256) {
        return (projectSubmissions[id].owner, projectSubmissions[id].state, projectSubmissions[id].index, projectSubmissions[id].minimumFunds, projectSubmissions[id].maximumFunds,proposals[id].votes.length);
    }

    function getAllProjectSubmissionKeys() public view returns(bytes[] memory) {
        return projectSubmissionKeys;
    }

    function updateMinimuProjectFunds(bytes memory id, uint256 minFunds) daoActive memberOnly public payable returns(bool) {
        require(msg.sender != address(0), "invalid address");
        require(projectSubmissions[id].exists, "project doesnt exist");
        require(projectSubmissions[id].state != State.Completed, "project already complted");
        require(projectSubmissions[id].state != State.AssesmentPhase, "project in AssementPhase");
        require(projectSubmissions[id].canUpdateFundsRequirement, "project funds cannot be altered");
        require(projectSubmissions[id].minimumFunds != minFunds, "funds must not be the same");
        require(msg.value == fundsUpdateFee, "insufficient funds");
        projectSubmissions[id].minimumFunds = projectSubmissions[id].minimumFunds.add(minFunds);
        withdrawableByDaoOwner = withdrawableByDaoOwner.add(msg.value);
        emit UpdateProjectMinFunds(id, minFunds);
    }

    function updateMaxProjectFunds(bytes memory id, uint256 maxFunds) daoActive memberOnly public payable returns(bool) {
        require(msg.sender != address(0), "invalid address");
        require(projectSubmissions[id].exists, "project doesnt exist");
        require(projectSubmissions[id].state != State.Completed, "project already complted");
        require(projectSubmissions[id].state != State.AssesmentPhase, "project in AssementPhase");
        require(projectSubmissions[id].canUpdateFundsRequirement, "project funds cannot be altered");
        require(projectSubmissions[id].maximumFunds != maxFunds, "funds must not be the same");
        projectSubmissions[id].maximumFunds = projectSubmissions[id].maximumFunds.add(maxFunds);
        withdrawableByDaoOwner = withdrawableByDaoOwner.add(msg.value);
        emit UpdateProjectMaxFunds(id, maxFunds);
    }

    function lowerSubmissionFee(uint256 _fee) onlyOwner daoActive external {
        require(_fee < submissionZeroFee, "fee must be lower");
        submissionZeroFee = _fee;
        emit FeeChanged(_fee);
    }

    function increaseSubmissionFee(uint256 _fee) onlyOwner daoActive external {
        require(_fee > submissionZeroFee, "fee must be greater");
        submissionZeroFee = _fee;
        emit FeeChanged(_fee);
    }

    function updateProjectState(bytes memory id, State state) daoActive public returns(bool) {
        require(msg.sender != address(0), "invalid address");
        require(state == State.Rejected || state == State.InProgress || state == State.AssesmentPhase || state == State.Completed, "Invalid state");
        require(projectSubmissions[id].exists, "project doesnt exist");
        require(projectSubmissions[id].state != State.Completed, "project already complted");
        require(projectSubmissions[id].canChangeState, "state cannot be changed");
        projectSubmissions[id].state = state;
        emit ProjectStateChanged(state);
    }

    function () daoActive external payable {
    ...
    function buyTokensInternal(address _buyer, uint256 _wei) daoActive internal {
