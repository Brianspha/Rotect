...
contract RotectDAO is Ownable {
    ...
    function blacklistAddress(address offender) internal daoActive {
        ...
        token.increaseLockedAmount(offender, token.getUnlockedAmount(offender));
    ...
    function removeFromBlackList(address offender) public payable daoActive {
        ...
        require(notVoting(offender), "Offender must not be involved in a vote");
        ...
        )
    {
        return (
            projectSubmissions[id].owner,
            uint256(projectSubmissions[id].state),
            projectSubmissions[id].index,
    ...
    function updateDeadLineDays(uint256 newDuration)
        public
        daoActive
        onlyOwner
    {
