...
let accounts;
const skynetId = web3.utils.asciiToHex("IADUs8d9CQjUO34LmdaaNPK_STuZo24rpKVfYW3wPPM2uQ")
...
contract('RotectDAO ', () => {
    it('should init token', async () => {
        var receipt = await RotectToken.methods.initialize("RToken", "RT", 18, new bigNumber(50000000000000000000000000000).toFixed()).send({
            gas: 8000000
        })
    })
    it('should transfer 2000000 tokens to member0', async () => {
        var receipt = await RotectToken.methods.transfer(member0, new bigNumber(2000000000000000000000000).toFixed()).send({
            gas: 8000000
        })
    })
    it('should transfer 2000000 tokens to member1', async () => {
        var receipt = await RotectToken.methods.transfer(member1, new bigNumber(2000000000000000000000000).toFixed()).send({
            gas: 8000000
        })
    })
    it('should transfer 2000000 tokens to member2', async () => {
        var receipt = await RotectToken.methods.transfer(member2, new bigNumber(2000000000000000000000000).toFixed()).send({
            gas: 8000000
        })
    })
    it('should transfer 2000000 tokens to member3', async () => {
        var receipt = await RotectToken.methods.transfer(member3, new bigNumber(2000000000000000000000000).toFixed()).send({
            gas: 8000000
        })
    })
    it('should transfer 2000000 tokens to member4', async () => {
        var receipt = await RotectToken.methods.transfer(member4, new bigNumber(2000000000000000000000000).toFixed()).send({
            gas: 8000000
        })
    })
...
contract('RotectDAO ', () => {
    ...
    it('should initialize the DAO', async () => {
        const receipt = await RotectDAO.methods.initialize(RotectToken.options.address).send({
            gas: 8000000
        })
    })
    ...
    it('should add Project Submission', async () => {
        const receipt = await RotectDAO.methods.addProjectSubmission(skynetId, 2500, 5000).send({
            gas: 8000000,
    ...
    it('should propose', async () => {
        const receipt = await RotectDAO.methods.propose(skynetId, "Proposal to accept project", 2).send({
            gas: 8000000,
        })
        console.log(receipt.events.ProposalAdded.returnValues)
    })
    it('should vote', async () => {
        const receipt = await RotectDAO.methods.vote(0, true, "Proposal to accept project", 200).send({
            gas: 8000000
        })
        assert.strictEqual(receipt, true)
    ...
    it(' should update Minimum Project Funds', async () => {
        const receipt = await RotectDAO.methods.updateMinimuProjectFunds(skynetId,1000).send({
            gas: 8000000,
            value: web3.utils.toWei('2', 'ether')
        })
    ...
    it(' should update Max Project Funds', async () => {
        const receipt = await RotectDAO.methods.updateMaxProjectFunds(skynetId,1000).send({
            gas: 8000000,
            value: web3.utils.toWei('2', 'ether')
        })
