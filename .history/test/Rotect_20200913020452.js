...
contract("RotectDAO ", () => {
  ...
  it("Should propose to accept a project", async () => {
    const receipt = await RotectDAO.methods
      .propose(skynetId, "Proposal to accept project", 2)
      .send({});
    assert.strictEqual(receipt !== null, true);
    console.log("ProposalAdded: ", receipt.events.ProposalAdded.returnValues);
  });
  it("Should vote", async () => {
    const receipt = await RotectDAO.methods
      .vote(0, true, "Vote to accept project", 200)
      .send({});
    console.log("Voted: ", receipt.events.Voted.returnValues);
    assert.strictEqual(receipt !== null, true);
  });
  ...
  it("Should get a project submission", async () => {
    ...
    var project = await RotectDAO.methods.getProjectSubmission(skynetId).call();
  ...
  it("Should update Minimum Project Funds", async () => {
    ...
    assert.strictEqual(receipt !== null, true);
    console.log(
      "UpdateProjectMinFunds: ",
      receipt.events.UpdateProjectMinFunds.returnValues
    );
  });
  it("Should update Max Project Funds", async () => {
    ...
    assert.strictEqual(receipt !== null, true);
    console.log(
      "UpdateProjectMaxFunds: ",
      receipt.events.UpdateProjectMaxFunds.returnValues
    );
  });
