 ...
 */
contract RotectToken is Initializable, LockableToken, IERC20 {
    using SafeMath
    for uint256;
     ...
     * construction.
     */
    function initialize(string memory name, string memory symbol, uint256 decimals, uint256 supply) public initializer {
    ...
    function increaseLockedAmount(address _owner, uint256 _amount)  public returns(uint256) {
        ...
        require(balanceOf(_owner) >= lockingAmount, "Locking exceeds balance");
    ...
    function decreaseLockedAmount(address _owner, uint256 _amount)  public returns(uint256) {
        uint256 amt = _amount;
        require(locked[_owner] > 0, "0 locked tokens.");
